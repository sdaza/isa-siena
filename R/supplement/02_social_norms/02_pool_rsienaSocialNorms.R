########################################################
# pool rsiena results using the social norm mechanism
...
# libraries
# install library before running the code
library(sdazar)
library(RSienaTest)
library(texreg)
library(igraph)
...
\\centering
\\scriptsize
\\caption{Social Norms. Coverage Rate, Bias and Siena Estimates by \\newline Scenario and Specification (", max_rep, " replicates)} \\label{suppl:norms}
...
clean_latex_rows <- list.clean(latex_rows)

# create and save table
cat(top, do.call(paste, clean_latex_rows) , bottom, file = paste0("latex/tables/results_norms.tex"))
